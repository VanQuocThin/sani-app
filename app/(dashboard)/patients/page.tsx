...
type Patient = { id: string; name: string; phone: string; dob: string; gender: string; medical_history: string; created_at: string };
...
export default function PatientsPage() {
  ...
  return (
    <div className="min-h-full">
      <Header title="Bệnh nhân" subtitle={`${patients.length} bệnh nhân`}
        action={<button onClick={() => setShowModal(true)} className="bg-[#00b96b] hover:bg-[#009958] text-white text-sm font-semibold px-4 py-2 rounded-lg transition-colors">+ Thêm bệnh nhân</button>}
      />
      <div className="p-6">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3"><p className="text-sm text-slate-600">{p.dob ? new Date(p.dob).toLocaleDateString("vi-VN") : "—"}</p><p className="text-xs text-slate-400">{p.gender ?? "—"}</p></td>
                    <td className="px-5 py-3 text-sm text-slate-500">{p.medical_history || "—"}</td>
                    <td className="px-5 py-3 text-sm text-slate-400">{new Date(p.created_at).toLocaleDateString("vi-VN")}</td>
                    <td className="px-5 py-3"><button className="text-xs text-[#00b96b] hover:underline font-medium">Xem hồ sơ</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              <div><label className="block text-xs font-semibold text-slate-600 mb-1">Họ và tên *</label>
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nguyễn Thị A"
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b]" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Số điện thoại *</label>
                  <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="0912 345 678"
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b]" /></div>
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Ngày sinh</label>
                  <input type="date" value={form.dob} onChange={(e) => setForm({ ...form, dob: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b]" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Giới tính</label>
                  <select value={form.gender} onChange={(e) => setForm({ ...form, gender: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b]">
                    <option>Nam</option><option>Nữ</option><option>Khác</option></select></div>
                <div><label className="block text-xs font-semibold text-slate-600 mb-1">Số BHYT</label>
                  <input type="text" value={form.bhyt_number} onChange={(e) => setForm({ ...form, bhyt_number: e.target.value })} placeholder="DN4012345678"
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b]" /></div>
              </div>
              <div><label className="block text-xs font-semibold text-slate-600 mb-1">Tiền sử bệnh</label>
                <textarea rows={2} value={form.medical_history} onChange={(e) => setForm({ ...form, medical_history: e.target.value })} placeholder="Dị ứng thuốc, bệnh mãn tính..."
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-[#00b96b] resize-none" /></div>
            </div>
